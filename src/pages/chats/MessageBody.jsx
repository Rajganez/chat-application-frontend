...
const MessageBody = () => {
  ...
  //Redux state variable to get the actions
  ...
  const getMsgfromDB = useCallback(() => {
    const getMsg = async () => {
  ...
  const renderMsg = () => {
    let lastDate = null;
    return messagesfromDB.map((msg, ind) => {
  ...
  //Function to Seperate the Sender and Recipient and different action for the different fileTypes
  const renderDM = (msg) =>
    msg.senderId === buddyiesInfo.buddyId ? (
      <div key={msg._id}>
            ...
            {(msg.messageType === "file" &&
              (/\.(jpg|png|jpeg|gif|bmp|tiff|webp|svg|ico|heic|heif)$/i.test(
                ...
                <div style={{ cursor: "pointer" }}>
                  <span
                    onClick={() =>
                      handleDownload(msg.content, extractFileName(msg.content))
                    }
        ...
        <div className="d-flex justify-content-end text-secondary font-size-timestamp">
      ...
      <div key={msg._id}>
        ...
        <div className="d-flex justify-content-start text-secondary font-size-timestamp">
          {moment(msg.timestamp).format("LT")}
        </div>
        {imageView !== null && (
          <div
